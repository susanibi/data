...
for group, output_file in plot_configs:
    ...
    # Instead of using ax.set_title, use fig.text to position the title exactly where needed
    # This gives more control over title placement
    # x=0.5 centers horizontally, y=0.9 positions near the top of the figure
    fig.text(0.5, 0.93, f"Surface Type ({group})",
    ...
    for j, load in enumerate(loads):
        ...
        ax.text(x[j], max_y + 1.0, f"t={row['t']}\np={row['p']:.2e}\n{stars}", ha='center', fontsize=small_fontsize)
