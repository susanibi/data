...
def shorten_sample_name(path):
    ...
    parts = base.split('_')
    return '_'.join(parts[-3:])  # ex: 3p5_50pg_ACN
...
def analyze_gravy_psm():
    ...
    for file_path in tqdm(pr_files, desc="Processing files"):
        try:
            ...
            # Find raw file columns (abundance)
            ...
            # Keep only what's needed for downstream
            df = df[["GRAVY", "PSM_Like_Count"] + sample_cols].copy()
            peptide_psm_summary[subfolder_name] = (df, sample_cols)
        except Exception as e:
            print(f"Error processing {file_path}: {e}")

    # Create consistent GRAVY bins
    ...
    for name, (df, sample_cols) in peptide_psm_summary.items():
        ...
        # Summed PSM per bin per replicate
