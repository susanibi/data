...
def main():
    ...
    for name, df in raw_abundance_dict.items():
        ...
        abundance = df.groupby("GRAVY_bin", observed=True)[non_empty_cols].median()
    ...
    for name, df in raw_abundance_dict.items():
        ...
        valid_cols = valid_cols.drop('GRAVY_bin', errors='ignore')

        df_filtered = df[["GRAVY_bin"] + list(valid_cols)]
        assert df_filtered.columns.duplicated().sum() == 0, "Duplicate column detected!"

        # Fill NaNs with 0 before stats
        filled = df_filtered[valid_cols].fillna(0)
        grouped_filled = filled.groupby(df_filtered["GRAVY_bin"], observed=True)

        # Calculate stats using zeros
        median_abund = grouped_filled.median()
        std_abund = grouped_filled.std(ddof=1)
        count_abund = grouped_filled.count()
        sem_abund = std_abund / np.sqrt(count_abund)

        # Ensure clean numeric types
        median_abund = median_abund.astype(float)
        sem_abund = sem_abund.astype(float)

        # Still count how many replicates were originally missing
        ...
        summary_abund = pd.DataFrame({
            "GRAVY_bin": median_abund.index,
            f"{name}_Median": median_abund.median(axis=1).values,
            f"{name}_SEM": sem_abund.mean(axis=1).values,
            f"{name}_ZeroCount": zero_count_abund.values
        })
        median_abundance_records.append(summary_abund)
        ...
        if name in psm_percent_dict:
            ...
            mean_psm_global = pd.to_numeric(mean_psm_global, errors='coerce')
            sem_psm_global = pd.to_numeric(sem_psm_global, errors='coerce')
            ...
            median_psm_records.append(summary_psm_global)

        print(f"✅ Finished {name}")

    from functools import reduce
    df_median_abundance_all = reduce(lambda left, right: pd.merge(left, right, on="GRAVY_bin", how="outer"), median_abundance_records)
    df_psm_mean_all = reduce(lambda left, right: pd.merge(left, right, on="GRAVY_bin", how="outer"), median_psm_records)
    ...
    summary_path = os.path.join(selected_dir, "Output_Combined_Median_Abundance_and_%PSM.xlsx")
    ...
    with pd.ExcelWriter(summary_path) as writer:
        df_median_abundance_all.to_excel(writer, sheet_name="Median_Abundance", index=False)
